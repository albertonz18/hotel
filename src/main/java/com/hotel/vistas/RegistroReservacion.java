...
public class RegistroReservacion extends JFrame {
	...
	private JTextField txHospedaje;
	...
	public RegistroReservacion( String idCliente) {
		...
		JLabel labelTitulo = new JLabel("Reservar");
		...
		txHospedaje.setEditable(false);
		txHospedaje.setBounds(278, 181, 182, 25);
		paneReservar.add(txHospedaje);
		txHospedaje.setColumns(10);
		...
		txReservacion.setEditable(false);
		...
		btnReservar.setFont(new Font("Bahnschrift", Font.PLAIN, 16));
		btnReservar.setBackground(new Color(200,150,41));
		btnReservar.setBounds(316, 327, 148, 32);
		paneReservar.add(btnReservar);
		btnReservar.setOpaque(isOpaque());
			...
			public void actionPerformed(ActionEvent e) {
				...
				if(dateEntrada.getDate() != null && 
				   ...
				   txTotal.getText().length() != 0) {
					...
					Integer diasHospedaje = Integer.valueOf(txHospedaje.getText());
					Double costoTotal = Double.valueOf(txTotal.getText());
					String nReserva = txReservacion.getText();
					Integer nCliente = Integer.valueOf(txCliente.getText());
					
					DatosReserva nvaReserva = new DatosReserva(fEntrada,fSalida, formaPago, diasHospedaje, costoTotal, nReserva);
					
					ReservaDAO.guardar(nvaReserva,nCliente, empresa);
					dispose();
		...
		btnReservar.addActionListener(reserva);
		
		JLabel labelFondoReserva = new JLabel("");
		labelFondoReserva.setBounds(-18, 0, 633, 380);
		labelFondoReserva.setIcon(new ImageIcon(RegistroReservacion.class.getResource("/com/hotel/imagenes/recepcion.png")));
