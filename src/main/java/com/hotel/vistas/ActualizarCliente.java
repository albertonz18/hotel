...
public class ActualizarCliente extends JFrame{
	...
	public ActualizarCliente(int idCliente,String nombre,String apellido,String fNAcimiento, String correo) {
		...
		JLabel labelCuadroAcceso = new JLabel("Actualizar Cliente");
		...
		txNumCliente.setText(String.valueOf(idCliente));
		txNombre.setText(nombre);
		txApellido.setText(apellido);
		dateNacimiento.setDate(java.sql.Date.valueOf(fNAcimiento));
		txCorreoCliente.setText(correo);
			...
			public void actionPerformed(ActionEvent e) {
				...
				if(txNombre.getText().length() != 0 && 
				   ...
				   txCorreoCliente.getText().length() != 0) {
					...
					String correo = txCorreoCliente.getText();
					String id = txNumCliente.getText();
					
					ClienteDAO.actualizarRegistro(id, nombre, apellido, fNacimiento, correo);
		...
		lblNewLabel.setIcon(new ImageIcon(ActualizarCliente.class.getResource("/com/hotel/imagenes/registro-hotel.jpg")));
